...
WS=/root/ros2_ws
PKG="$WS/src/frontier_safety"

echo ">> Creating package skeleton at $PKG"
mkdir -p "$PKG/frontier_safety" "$PKG/launch" "$PKG/resource"

# --- ament resource marker ---
touch "$PKG/resource/frontier_safety"

# --- python package init ---
touch "$PKG/frontier_safety/__init__.py"

# --- package.xml ---
cat > "$PKG/package.xml" << 'XMLEOF'
<?xml version="1.0"?>
<package format="3">
  <name>frontier_safety</name>
  <version>0.1.0</version>
  <description>Reactive LiDAR collision avoidance (gaussian polar histogram) for the sjtu_drone frontier explorer.</description>
...
[develop]
script_dir=$base/lib/frontier_safety
[install]
install_scripts=$base/lib/frontier_safety
CFGEOF

# --- launch file ---
cat > "$PKG/launch/collision_avoidance.launch.py" << 'LAUNCHEOF'
#!/usr/bin/env python3
...
echo ">> Writing collision_avoidance.py"
cat > "$PKG/frontier_safety/collision_avoidance.py" << 'NODEEOF'
#!/usr/bin/env python3
...
echo ">> Building frontier_safety"
cd "$WS"
colcon build --symlink-install --packages-select frontier_safety
...
>> Done. To use it:
   ...
   IMPORTANT: point your command source at cmd_vel_raw (not cmd_vel), e.g.
   teleop:  ros2 run <teleop_pkg> <teleop> --ros-args -r /simple_drone/cmd_vel:=/simple_drone/cmd_vel_raw

   Quick wall test:
   ros2 topic pub /simple_drone/cmd_vel_raw geometry_msgs/msg/Twist "{linear: {x: 1.0}}" -r 10
   ros2 topic echo /collision_avoidance/emergency_stop
DONEEOF
